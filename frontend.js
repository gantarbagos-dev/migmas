...
async function kickSelectedTargets(){
  ...
  const step = el("kickProgressStep"), ws = el("kickProgressWs"), structure = el("kickProgressStructure");
  ...
  try{
    ...
    const readProgress = async () => {
      ...
      try{
        ...
        if(Array.isArray(p.wsProgress)) {
          const startedWs = p.wsProgress.filter(x => (Number(x.completed) || 0) > 0).length;
          ws.textContent = `WS ${startedWs}/${p.wsProgress.length || wsCount}`;
        } else if(p.phase === "completed") {
          ws.textContent = `WS ${wsCount}/${wsCount}`;
        }
